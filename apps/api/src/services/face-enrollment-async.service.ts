...
async function getServerHuman(forceCpu=false){
  if(forceCpu){humanPromise=null;humanBackend='cpu';}
  if(!humanPromise){
    humanPromise=(async()=>{
      if(humanBackend==='cpu')return buildHuman('cpu');
      try { return await buildHuman('wasm'); }
      catch (wasmError) {
        console.warn('[PontoProof] Face worker WASM indisponível; usando CPU em segundo plano.',wasmError);
        humanBackend='cpu';
        return buildHuman('cpu');
      }
    })().catch(err=>{humanPromise=null;throw err;});
  }
  return humanPromise;
}
...
async function analyzePhoto(photo:StoredPhoto):Promise<ProcessedPhoto>{
  ...
  try{return await detectDecoded(photo,decoded,human);}
  catch(error){
    if(humanBackend==='wasm'){
      console.warn('[PontoProof] Falha na inferência WASM; refazendo esta análise com CPU.',error);
...
export async function createAsyncFaceEnrollment(input:{tenantId:string;employeeId:string;userId:string;photos:EnrollmentPhoto[]}){
  ...
  try{
    ...
    return await prisma.$transaction(async tx=>{
      ...
      await tx.auditEvent.create({data:{tenantId:input.tenantId,actorUserId:input.userId,action:'FACE_ENROLLMENT_SUBMITTED',entityType:'FaceEnrollmentSubmission',entityId:submission.id,metadataJson:{poses:required,processing:'ASYNC_SERVER'}}});
      return submission;
    });
...
async function approveSubmission(submission:any, photos:ProcessedPhoto[]){
  await prisma.$transaction(async tx=>{
    ...
    await tx.auditEvent.create({data:{tenantId:submission.tenantId,actorUserId:submission.userId,action:'FACE_ENROLLMENT_APPROVED_ASYNC',entityType:'FaceEnrollmentSubmission',entityId:submission.id,metadataJson:{quality:photos.map(p=>({pose:p.pose,quality:p.quality,liveness:p.liveness,antiSpoof:p.antiSpoof}))}}});
  });
}
...
async function processSubmission(submission:any){
  ...
  try{
    ...
    await prisma.auditEvent.create({data:{tenantId:row.tenantId,actorUserId:row.userId,action:'FACE_ENROLLMENT_PROCESSING_FAILED',entityType:'FaceEnrollmentSubmission',entityId:row.id,metadataJson:{message}}});
  }
}
...
export async function retryFaceEnrollmentSubmission(input:{tenantId:string;userId:string;submissionId:string}){
  ...
  return prisma.faceEnrollmentSubmission.update({where:{id:row.id},data:{status:'PENDING',progress:5,reasonsJson:null,processedAt:null,processingStartedAt:null}});
}
